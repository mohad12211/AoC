...
impl FromStr for Point {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let components: Vec<_> = s
            ...
            .map(|s| s.parse())
            .collect::<Result<_, _>>()
            .map_err(|_| "Expected numbers separatd by ,")?;
        if components.len() != 3 {
            Err("Expected 3 numbers".into())
        } else {
            Ok(Self {
                x: components[0],
                y: components[1],
...
fn main() {
    ...
    for brick in non_removables {
        total += get_fall_count(&brick, stack.clone());
    }
    println!("{total}");
}

fn get_fall_count(removed: &Brick, mut stack: HashMap<usize, Vec<Brick>>) -> usize {
    ...
    let row = stack.values_mut().find(|r| r.contains(removed)).unwrap();
    row.swap_remove(row.iter().position(|x| x == removed).unwrap());

    for z in removed.end.z..=max_z {
        let Some(current_bricks) = stack.get(&z) else {
            continue;
        };
        let mut falling_bricks = Vec::new();
        for (brick_index, brick) in current_bricks.iter().enumerate() {
            let Some(bricks_below) = stack.get(&(z - brick.vertical_len())) else {
                continue;
            };

            if brick.can_fall_to(bricks_below) {
                falling_bricks.push(brick_index);
            }
        }

        fall_count += falling_bricks.len();
        for &brick_index in falling_bricks.iter().rev() {
            stack.get_mut(&z).unwrap().swap_remove(brick_index);
        }
    }
